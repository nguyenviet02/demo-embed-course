...
export default function DeckEntry({ deck, onEnter, isVerified = false, defaultLang, onResetPassword, defaultNarration }: DeckEntryProps) {
    const [selectedLang, setSelectedLang] = useState(defaultLang || deck.languages[0]);
    const [password, setPassword] = useState('');
    const [error, setError] = useState('');
    const [loading, setLoading] = useState(false);

    // Narration state - default to true if available for selected language
    const hasNarrationForLang = deck.narration?.languages.includes(selectedLang) ?? false;
    const [narrationEnabled, setNarrationEnabled] = useState(defaultNarration ?? hasNarrationForLang);

    // Helper to sort languages moving user's preference to top
    const getSortedLanguages = useCallback((topLang: string) => {
        if (!deck.languages.includes(topLang)) return deck.languages;
        return [...deck.languages].sort((a, b) => (a === topLang ? -1 : b === topLang ? 1 : 0));
    }, [deck.languages]);
    ...
    }, [needsPassword, deck.id, password, selectedLang, narrationEnabled, onEnter, onResetPassword]);
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {sortedLanguages.map((lang) => {
                                    const { flag, name } = languageLabels[lang] || { flag: '🌐', name: lang };
                                    return (
                                        <button
                                            key={lang}
                                            type="button"
                                            onClick={() => setSelectedLang(lang)}
                                            className={`p-4 rounded-xl border-2 transition-all ${selectedLang === lang
                    ...
                    {hasNarrationForLang && (
                        <div className="flex items-center justify-between p-4 bg-gray-700/50 rounded-xl border border-gray-600">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-sm font-medium text-white">Thuyết minh</div>
                                    <div className="text-xs text-gray-400">Tự động phát âm thanh</div>
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={() => setNarrationEnabled(!narrationEnabled)}
                                className={`relative w-12 h-6 rounded-full transition-colors ${narrationEnabled ? 'bg-cyan-500' : 'bg-gray-600'}`}
                            >
                                <span
                                    className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${narrationEnabled ? 'translate-x-6' : 'translate-x-0'}`}
                                />
                            </button>
                        </div>
                    )}
