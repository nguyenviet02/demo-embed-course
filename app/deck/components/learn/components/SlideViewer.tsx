...
export default function SlideViewer({
  ...
  const { submitCourse } = useSignatureAuth({ signature: signature || "" });
      ...
      if (transition?.type === "ripple") {
        ...
        if (transition.originSelector) {
          ...
          const originElement = document.querySelector(
            transition.originSelector
          );
          if (originElement) {
            const circle = originElement.querySelector(
              ".rounded-full"
            ) as HTMLElement;
  ...
  useEffect(() => {
    const buttonSubmit = document.getElementById("submitCourse");
    if (buttonSubmit) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`hidden md:block absolute top-6 left-6 z-50 transition-opacity duration-300 ${
      ...
      <div
        className={`absolute top-6 right-6 z-50 flex gap-2 transition-opacity duration-300 ${
      ...
      >
        ...
        {audioBasePath && (
          <button
            ...
            aria-label={
              narrationEnabled ? "Tắt thuyết minh" : "Bật thuyết minh"
            }
          ...
          >
            {narrationEnabled ? (
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className={`w-5 h-5 ${isAudioPlaying ? "animate-pulse" : ""}`}
              >
                <path
                  strokeLinecap="round"
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M17.25 9.75 19.5 12m0 0 2.25 2.25M19.5 12l2.25-2.25M19.5 12l-2.25 2.25m-10.5-6 4.72-4.72a.75.75 0 0 1 1.28.53v15.88a.75.75 0 0 1-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 0 1 2.25 12c0-.83.112-1.633.322-2.396C2.806 8.757 3.63 8.25 4.51 8.25H6.75Z"
                />
              </svg>
            )}
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 bg-black/30 hover:bg-white/20 backdrop-blur-md border border-white/10 rounded-full text-white/50 hover:text-white transition-all duration-300 group"
          aria-label={
            course.contentType === "deck" ? "Restart Deck" : "Exit Course"
          }
        ...
        >
          {course.contentType === "deck" ? (
            ...
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-5 h-5 md:w-5 md:h-5 group-hover:rotate-180 transition-transform duration-500"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-5 h-5 md:w-5 md:h-5 group-hover:rotate-90 transition-transform"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      <button
        ...
        className={`absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-40 p-2 md:p-3 text-white/20 hover:text-white/80 hover:bg-white/5 rounded-full transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none ${
          showControls ? "opacity-100" : "opacity-0"
        }`}
      >
        <svg
      ...
      <button
        ...
        className={`absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-40 p-2 md:p-3 text-white/20 hover:text-white/80 hover:bg-white/5 rounded-full transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none ${
          showControls ? "opacity-100" : "opacity-0"
        }`}
      >
        <svg
      ...
      <style jsx global>{`
        ...
        .animate-zoom-in-gate {
          animation: zoom-in-gate 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94)
            forwards !important;
