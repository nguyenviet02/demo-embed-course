...
export type { Block, HeadingBlock, TextBlock, ImageBlock, HtmlBlock } from '../learn/types';

export type SlideLayout = 'default' | 'cover' | 'section-header' | 'split' | 'centered' | 'three-column' | 'reveal' | 'carousel-3d';
...
export interface DeckSlide {
    ...
    imageUrl?: string;
    showBadge?: boolean;
    blocks?: import('../learn/types').Block[];
    columns?: {
        title: string;
        content: string;
        icon?: string;
    ...
    carouselItems?: { // For carousel-3d layout
        title: string;
        desc?: string;
        icon?: string;
        imageUrl?: string;
